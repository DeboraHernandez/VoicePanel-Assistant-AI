...
def _apply_select_ops(converter):
    """
    Configura SELECT_TF_OPS en el converter.

    La Bi-LSTM usa ops TensorList internamente (para manejar los estados
    recurrentes). Estas ops NO están en el conjunto estándar de TFLite
    (TFLITE_BUILTINS) y sin SELECT_TF_OPS la conversión falla con:
        "Some ops are not supported by the standard TFLite runtime"

    _experimental_lower_tensor_list_ops = False es obligatorio para que
    TFLite no intente reemplazar las TensorList ops (lo que rompe la LSTM).

    Referencia: https://www.tensorflow.org/lite/guide/ops_select
    """
    import tensorflow as tf
...
def export_tflite(model, output_path, X_train_sample=None):
    """
    Exporta el modelo Keras a TFLite.

    Intenta cuantización INT8 completa (más rápida en RPi4 con NEON).
    Si falla, hace fallback a cuantización dinámica de pesos.
    En ambos casos aplica SELECT_TF_OPS (obligatorio para Bi-LSTM).
    """
    import tensorflow as tf
    ...
    if X_train_sample is not None:
        try:
            ...
            quant_type   = "INT8 (con SELECT_TF_OPS)"

        except Exception as e:
            print(f"  ⚠ INT8 falló ({e})\n    Usando cuantización dinámica...")
            converter2 = tf.lite.TFLiteConverter.from_keras_model(model)
            converter2.optimizations = [tf.lite.Optimize.DEFAULT]
            _apply_select_ops(converter2)   # ← siempre necesario para Bi-LSTM
            tflite_model = converter2.convert()
            quant_type   = "Dinámica de pesos (con SELECT_TF_OPS)"
    else:
        _apply_select_ops(converter)        # ← siempre necesario para Bi-LSTM
        tflite_model = converter.convert()
        quant_type   = "Dinámica de pesos (con SELECT_TF_OPS)"
...
def benchmark_tflite(tflite_path, X_sample, n_runs=50):
    ...
    Si el intérprete TFLite no tiene soporte para Flex ops
    (RuntimeError al allocate_tensors), hace fallback al modelo
    Keras .keras equivalente. La latencia medida sigue siendo
    válida como referencia para la comparativa CNN vs LSTM.
    """
    import tensorflow as tf
    import time
    ...
    try:
        ...
        print(f"  ⚠ TFLite Flex ops no disponibles en este runtime ({e})")
        print(f"    Fallback: midiendo latencia con modelo Keras (referencia válida).")
...
def plot_training_curves(history, out_dir, prefix="lstm"):
    ...
    for ax, metric, ylabel in [
        ...
        ax.plot(epochs, history[train_key], color="#3C3489", label="Train", linewidth=1.8)
        ax.plot(epochs, history[val_key],   color="#1D9E75", label="Val",   linewidth=1.8)
        ax.set_xlabel("Época")
...
def plot_cnn_vs_lstm(y_test, y_pred_cnn, y_pred_lstm,
    ...
    x     = np.arange(2)
    width = 0.35
    b1 = ax1.bar(x - width/2, [acc_cnn, acc_lstm], width, label="Accuracy",
                 color=["#3C3489", "#1D9E75"], alpha=0.85)
    b2 = ax1.bar(x + width/2, [f1_cnn,  f1_lstm],  width, label="F1 macro",
                 color=["#6B62C8", "#45C49A"], alpha=0.85)
    ax1.bar_label(b1, fmt="%.4f", fontsize=9, padding=3)
    ax1.bar_label(b2, fmt="%.4f", fontsize=9, padding=3)
    ax1.set_xticks(x)
    ax1.set_xticklabels(["CNN 1D", "Bi-LSTM"], fontsize=11)
    ax1.set_ylim([0, 1.12])
    ...
    ax3.plot(epochs, history_lstm["accuracy"],     color="#1D9E75", linewidth=1.8, label="LSTM train")
    ax3.plot(epochs, history_lstm["val_accuracy"], color="#1D9E75", linewidth=1.8,
             linestyle="--", label="LSTM val")
    if cnn_info and "val_accuracy" in cnn_info:
        ax3.axhline(cnn_info["val_accuracy"], color="#3C3489", linewidth=1.5,
                    linestyle=":", alpha=0.8,
    ...
    rows = [
        ...
        ["Parámetros",        f"{cnn_info.get('total_params',0):,}" if cnn_info else "N/A", "ver reporte"],
        ["TFLite (KB)",       f"{cnn_info.get('tflite_kb',0):.0f}" if cnn_info else "N/A", "ver reporte"],
        ["Latencia media*",   f"{latency_cnn['mean_ms']:.1f} ms"  if latency_cnn  else "N/A",
                              f"{latency_lstm['mean_ms']:.1f} ms" if latency_lstm else "N/A"],
        ["Latencia P95*",     f"{latency_cnn['p95_ms']:.1f} ms"   if latency_cnn  else "N/A",
                              f"{latency_lstm['p95_ms']:.1f} ms"  if latency_lstm else "N/A"],
        ["Épocas entrenadas", str(cnn_info.get("epochs_trained","N/A")) if cnn_info else "N/A",
                              str(len(history_lstm["accuracy"]))],
    ]
    colors_tbl = [["#3C3489"] * 3] + [["white"] * 3] * (len(rows) - 1)
    tbl = ax4.table(cellText=rows, cellLoc="center", loc="center", cellColours=colors_tbl)
...
def save_report(model, history, y_val, y_val_pred,
                y_test, y_test_pred, y_pred_cnn, classes,
    ...
    log()
    log("NOTA TÉCNICA — EXPORTACIÓN TFLITE BI-LSTM")
    log(sep2)
    log("  La Bi-LSTM usa ops TensorList internamente.")
    log("  Se requiere SELECT_TF_OPS en el converter (no estándar en CNN).")
    log("  El intérprete en RPi4 necesita tflite-runtime con Flex delegate.")
    log("  Instalar: pip install tflite-runtime  (más liviano que tensorflow)")
    ...
    log(f"  {'-'*22} {'-'*10} {'-'*10} {'-'*10}")
    log(f"  {'Accuracy (test)':<22} {acc_cnn:>10.4f} {acc_lstm:>10.4f} {acc_lstm-acc_cnn:>+10.4f}")
    log(f"  {'F1 macro (test)':<22} {f1_cnn:>10.4f} {f1_lstm:>10.4f} {f1_lstm-f1_cnn:>+10.4f}")

    if cnn_info:
        cnn_p  = cnn_info.get("total_params", 0)
        lstm_p = model.count_params()
        log(f"  {'Parámetros':<22} {cnn_p:>10,} {lstm_p:>10,} "
            f"({'+'  if lstm_p > cnn_p else ''}{lstm_p - cnn_p:,})")
        log(f"  {'TFLite (KB)':<22} {cnn_info.get('tflite_kb',0):>10.1f} {tflite_size_kb:>10.1f}")

    if latency_cnn and latency_lstm:
        log(f"  {'Latencia media*':<22} {latency_cnn['mean_ms']:>9.1f}ms {latency_lstm['mean_ms']:>9.1f}ms")
        log(f"  {'Latencia P95*':<22} {latency_cnn['p95_ms']:>9.1f}ms  {latency_lstm['p95_ms']:>9.1f}ms")
    ...
    log(sep2)
    if y_pred_cnn is not None:
        lat_ok = latency_cnn and latency_cnn["mean_ms"] <= (latency_lstm or {}).get("mean_ms", 999)
        if acc_cnn >= acc_lstm and lat_ok:
            log("  → Usar CNN 1D: igual o mayor accuracy Y menor latencia.")
            log("    LSTM queda documentado como alternativa secuencial.")
        elif acc_lstm > acc_cnn:
            log("  → Bi-LSTM supera al CNN en accuracy.")
            log("    Evaluar si la diferencia de latencia es aceptable en RPi4.")
        else:
            log("  → Accuracy similar. Preferir CNN por menor latencia en RPi4.")
    else:
        log("  → Comparativa manual necesaria. Ver módulo 6 para métricas CNN.")

    log()
    log("PRÓXIMO PASO")
    log(sep2)
    log("  Ejecutar gpio_controller.py (módulo 8) para probar actuadores.")
    log()
    log(sep)
...
def main():
    ...
    try:
        ...
        print(f"  GPUs: {len(gpus)} {'(acelerado)' if gpus else '(CPU — considera Colab)'}")
    except ImportError:
    ...
    print(f"\nEntrenando — máx. {TRAIN_CFG['epochs']} épocas, batch {TRAIN_CFG['batch_size']}...")
    ...
    if not args.no_tflite:
        print("\nExportando a TFLite (con SELECT_TF_OPS para Bi-LSTM)...")
        ...
        if not args.no_benchmark:
            ...
            print(f"  Media: {latency_lstm['mean_ms']:.2f} ms  |  P95: {latency_lstm['p95_ms']:.2f} ms")

    print("\nCargando CNN para comparativa...")
    cnn_info   = load_cnn_info(args.output)
    y_pred_cnn, _ = load_cnn_predictions(args.output, args.splits)

    latency_cnn = cnn_info.get("latency_ms") if cnn_info else None
    ...
    lstm_info = {
        ...
        "tflite_note"   : "Requiere SELECT_TF_OPS (Flex delegate) en tflite-runtime",
    }
    with open(os.path.join(args.output, "lstm_model_info.json"), "w") as f:
        json.dump(lstm_info, f, indent=2)

    save_report(model, history, y_val, y_val_pred, y_test, y_test_pred,
                y_pred_cnn, classes, tflite_size_kb, latency_lstm, latency_cnn,
    ...
    if not args.no_plots:
        ...
        plot_cnn_vs_lstm(
            ...
            y_test_pred,
            history, cnn_info, classes,
            latency_cnn, latency_lstm,
            reports_dir,
        )
